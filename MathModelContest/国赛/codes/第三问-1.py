...
for outpressure in out_p:
        ...
        print(outpressure)
        min_loss = 10000000
        for phase0 in range(50,60):
            print('phase0:',phase0)
            for phase in range(49,52):
                for T in range(1220,1223): #以1ms为单位
                    T = T/10
                    #for phase
                    w = 2*3.1415926/T
                    p = {0: 100}  # 压强p(t)
                    ...
                    try:
                        for time in range(1, int(T*100)):
                            ...
                            q2= qn_delta_p(p0, time, outpressure, phase*100)
                            p0 = p0+Q*E[time-1]/V-q-q2  #原压强加上进油增压减去两嘴降压
                            p[time] = p0
